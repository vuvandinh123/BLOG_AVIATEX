...
import { GetAllVisits, GetDataCountDashboard } from "../../../service/visits";
...
const Dashboard = () => {
  ...
  useEffect(() => {
    (async () => {
      ...
      const res2 = await GetDataCountDashboard();
      setData(res2.data);
      if (res) setVisits(res.data);
    })();
  }, []);
  return (
    <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
      <div className="md:col-span-2 lg:col-span-1">
        <div className="h-full py-8 px-6 space-y-6 rounded-xl border border-gray-200 bg-white">
          ...
          <div>
            <h5 className="text-xl text-gray-600 text-center">Lượt truy cập</h5>
          ...
          <table className="w-full text-gray-600">
            <tbody>
              <tr>
                <td className="py-2">Hôm qua</td>
                <td className="text-gray-500">{visits.yesterday}</td>
                <td>
                  <svg
              ...
              <tr>
                <td className="py-2">Tuần này</td>
                <td className="text-gray-500">{visits.thisWeek}</td>
                <td>
                  <svg
                    className="w-16 ml-auto"
                    viewBox="0 0 68 21"
              ...
              <tr>
                <td className="py-2">Tất cả</td>
                <td className="text-gray-500">{visits.allTime}</td>
                <td>
                  <svg
                    className="w-16 ml-auto"
                    viewBox="0 0 68 21"
      ...
      <div>
        <div className="h-full py-6 px-6 rounded-xl  border border-gray-200 bg-white">
          <div className="flex justify-center">
            <img src={icon1} className="w-40" alt="" />
          </div>
          <h5 className="text-xl text-gray-700"></h5>
          <div className="my-8 text-center">
            <h1 className="text-5xl text-center font-bold text-gray-800">
              {data.col1.total}
            </h1>
            <span className="text-gray-500">Bài viết đang hiển thị</span>
          </div>
          <table className="mt-6 -mb-2 w-full text-gray-600">
            <tbody>
              <tr>
                <td className="py-2">Bài viết</td>
                <td className="text-gray-500">{data.col1.posts}</td>
              </tr>
              <tr>
                <td className="py-2">Tuyển dụng</td>
                <td className="text-gray-500">{data.col1.recruitment}</td>
              </tr>
              <tr>
                <td className="py-2">Hoạt động</td>
                <td className="text-gray-500">{data.col1.activities}</td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      <div>
        <div className="lg:h-full py-8 px-6 text-gray-600 rounded-xl border border-gray-200 bg-white">
          <div className="flex justify-center">
            <img src={icon2} className="w-60" alt="" />
          </div>
          <div className="mt-6">
            <h5 className="text-xl text-gray-700 text-center"></h5>
            <div className="mt-2 flex justify-center gap-4">
              <h3 className="text-5xl font-bold text-gray-700">
                {data.col2.contact_pending}
              </h3>
            </div>
            <span className="block text-center text-gray-500">
              Liên hệ mới chờ phản hồi
            </span>
          </div>
          <table className="mt-6 -mb-2 w-full text-gray-600">
            <tbody>
              <tr>
                <td className="py-2">Đã phản hồi</td>
                <td className="text-gray-500">{data.col2.contact_active}</td>
              </tr>
              <tr>
                <td className="py-2">Chưa phản hồi</td>
                <td className="text-gray-500">{data.col2.contact_pending}</td>
              </tr>
              <tr>
                <td className="py-2">Tất cả</td>
                <td className="text-gray-500">{data.col2.total}</td>
              </tr>
            </tbody>
          </table>
        </div>
